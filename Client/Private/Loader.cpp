...
HRESULT CLoader::Loading_ObjectProtoType()
{
	...
	if (FAILED(pGameInstance->Add_Prototype(TEXT("DragonTrail"),
		CDragonTrail::Create(m_pDevice, m_pContext))))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_Prototype(TEXT("DragonFire1"),
		CDragonFire1::Create(m_pDevice, m_pContext))))
		return E_FAIL;
...
HRESULT CLoader::Loading_Shader()
{
	...
	if (FAILED(GI->Add_Prototype(LEVEL_STATIC, TEXT("Prototype_Component_Shader_EffectPoint"),
		CShader::Create(m_pDevice, m_pContext, TEXT("../Bin/ShaderFiles/Shader_EffectPoint.hlsl"), VTXPOINT_DECLARATION::Elements, VTXPOINT_DECLARATION::iNumElements))))
		return E_FAIL;
...
HRESULT CLoader::LoadModel(char * DatName)
{
	...
	while (true)
	{
		...
		else if (!strcmp(DatName, "DragonEffect"))
		{
			if (FAILED(GI->Add_Prototype(LEVEL_STATIC, ProtoName,
				CModel::Create(m_pDevice, m_pContext, ModelName, DatName))))
			{
