...
HRESULT CLevel_Stage3::Initialize()
{
	...
	if (FAILED(Ready_Load_Model("Level_Stage3")))
		return E_FAIL;

	PM->Get_PlayerPointer()->Create_Navigation("Level_Stage3");
	PM->Set_Level(LEVEL_STAGE3);
	/*if (FAILED(Ready_UI("Level_Stage4")))
		return E_FAIL;*/

	GI->StopAll();
	GI->PlayBGM(L"Stage3.ogg", 0.6f);
...
HRESULT CLevel_Stage3::Ready_Lights()
{
	...
	_vector vLightEye = { 100.f, 80.f, 30.f };
	_vector vLightAt = { 100.f, 10.f, 100.f };
...
HRESULT CLevel_Stage3::Ready_Load_AnimModel(char * DatName)
{
...
HRESULT CLevel_Stage3::Ready_UI(char * DatName)
{
	string FileSave = DatName;
...
HRESULT CLevel_Stage3::Ready_Load_Model(char * DatName)
{
	string FileSave = DatName;
	...
	while (true)
	{
		...
		InstanceInfo->eLevel = LEVEL_STAGE3;
		...
		if (FAILED(GI->Add_GameObjectToLayer(TEXT("InstanceModel"), LEVEL_STAGE3, L"Layer_ModelObject", InstanceInfo)))
		{
			wstring a = L"Please Load ProtoType";
			wstring b = a + ModelName;
			const _tchar* c = b.c_str();
