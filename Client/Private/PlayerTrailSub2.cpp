#include "stdafx.h"
#include "..\Public\PlayerTrailSub2.h"
#include "GameInstance.h"

CPlayerTrailSub2::CPlayerTrailSub2(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CMesh(pDevice, pContext)
{
	m_pModel = nullptr;
}

CPlayerTrailSub2::CPlayerTrailSub2(const CPlayerTrailSub2& rhs)
	: CMesh(rhs)
{
}

HRESULT CPlayerTrailSub2::Initialize_Prototype()
{
	__super::Initialize_Prototype();
	return S_OK;
}

HRESULT CPlayerTrailSub2::Initialize(void * pArg)
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, L"PlayerTrailSub2", TEXT("PlayerTrailSub2"), (CComponent**)&m_pModel)))
		return E_FAIL;
...
void CPlayerTrailSub2::Tick(_float fTimeDelta)
{

}

void CPlayerTrailSub2::LateTick(_float fTimeDelta)
{
...
HRESULT CPlayerTrailSub2::Render()
{
	if (nullptr == m_pModel ||
...
CMesh * CPlayerTrailSub2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPlayerTrailSub2*		pInstance = new CPlayerTrailSub2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CPlayerTrailSub2"));
...
CGameObject * CPlayerTrailSub2::Clone(void * pArg)
{
	CPlayerTrailSub2*		pInstance = new CPlayerTrailSub2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CPlayerTrailSub2"));
		Safe_Release(pInstance);
	}
