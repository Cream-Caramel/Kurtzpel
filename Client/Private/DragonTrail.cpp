#include "stdafx.h"
#include "..\Public\DragonTrail.h"
#include "GameInstance.h"

CDragonTrail::CDragonTrail(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CMesh(pDevice, pContext)
{
	m_pModel = nullptr;
}

CDragonTrail::CDragonTrail(const CDragonTrail& rhs)
	: CMesh(rhs)
{
}

HRESULT CDragonTrail::Initialize_Prototype()
{
	__super::Initialize_Prototype();
	return S_OK;
}

HRESULT CDragonTrail::Initialize(void * pArg)
{
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, L"DragonTrail", TEXT("DragonTrail"), (CComponent**)&m_pModel)))
		return E_FAIL;
...
void CDragonTrail::Tick(_float fTimeDelta)
{

}

void CDragonTrail::LateTick(_float fTimeDelta)
{
...
CMesh * CDragonTrail::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CDragonTrail*		pInstance = new CDragonTrail(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CDragonTrail"));
...
CGameObject * CDragonTrail::Clone(void * pArg)
{
	CDragonTrail*		pInstance = new CDragonTrail(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CDragonTrail"));
		Safe_Release(pInstance);
	}
