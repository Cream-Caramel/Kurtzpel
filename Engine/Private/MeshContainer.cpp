...
HRESULT CMeshContainer::Initialize_Prototype(const aiMesh * pAIMesh, _fmatrix PivotMatrix, CBinary* pBinary)
{
...
#pragma region VERTEXBUFFER
	...
	for (_uint i = 0; i < m_iNumVertices; ++i)
	{
		...
		pBinary->m_BinaryVector->NonAniVertexInfo.NonAniVerticesInfo.push_back(pVertices[i]);
	}
...
CMeshContainer * CMeshContainer::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const aiMesh * pAIMesh, _fmatrix PivotMatrix, CBinary* pBinary)
{
	CMeshContainer*			pInstance = new CMeshContainer(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(pAIMesh, PivotMatrix, pBinary)))
	{
