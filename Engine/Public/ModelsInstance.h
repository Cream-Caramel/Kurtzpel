...
class ENGINE_DLL CModelsInstance final : public CComponent
{
private:
	CModelsInstance(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CModelsInstance(const CModelsInstance& rhs);
	virtual ~CModelsInstance() = default;
...
private:
	...
	vector<class CModelInstanceContainer*>			m_Meshes;
	typedef vector<class CModelInstanceContainer*>	MESHES;
...
public:
	static CModelsInstance* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext,  const char* pLoadName, const char* pSavePath, vector<_float3*> Matrix, _fmatrix PivotMatrix = XMMatrixIdentity());
